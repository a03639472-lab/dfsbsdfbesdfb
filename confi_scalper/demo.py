...
Твоя цель — ловить реальные импульсы и извлекать прибыль ~0.5$ с 10$ депозита за день.
Игнорируй шум, ложные сигналы, флэт и малые движения.

LONG — открывать только если ВСЕ условия:
1. Bid imbalance ≥ 65%
2. BUY volume > SELL volume ≥1.7×
3. Momentum ≥ 0.05%
...
ФИЛЬТРЫ:
- Не торговать если моментум < 0.03% (флэт)
- Игнорировать >80% сигналов
- Входи только в сильные импульсы
